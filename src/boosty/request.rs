...
impl Client {
    ...
    /// println!("{:?}", text);
    /// ```
    pub async fn request(method: String, auth: Option<Auth>) -> Result<Response, reqwest::Error> {
    ...
    ///     let posts = imgdl_rs::boosty::request::Client::fetch_posts("crptmem".to_string(), 300, None).await?;
    ///     println!("{:?}", posts); 
    ///     Ok(())
    /// }
    /// ```
    pub async fn fetch_posts(blog_name: String, limit: i64, auth: Option<Auth>) -> Result<Vec<Post>, serde_json::Error> {
        ...
        let json: Value = Self::request(url.clone(), auth).await.unwrap().json().await.unwrap();
    ...
    /// #[tokio::main]
    /// async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ///     let posts = imgdl_rs::boosty::request::Client::fetch_posts_raw("crptmem".to_string(), 300, None).await?;
    ///     println!("{:?}", posts); 
    ///     Ok(())
    /// }
    /// ```
    ...
    /// #[tokio::main]
    /// async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ///     let post = imgdl_rs::boosty::request::Client::fetch_post("boosty".to_string(), "c9fb8a19-c45e-4602-9942-087c3af28c1b".to_string(), None).await?;
    ///     println!("{:?}", post); 
    ///     Ok(())
    /// }
    /// ```
    pub async fn fetch_post(blog_name: String, post_id: String, auth: Option<Auth>) -> Result<Post, serde_json::Error> {
        ...
        let post: Post = Self::request(url, auth).await.unwrap().json().await.unwrap();
        Ok(post)
    }
}
